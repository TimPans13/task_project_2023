...
{
    public class QueenPathfinder : IChessGridNavigator
    {
        public List<Vector2Int> FindPath(ChessUnitType unit, Vector2Int from, Vector2Int to, ChessGrid grid)
        {
            if (unit != ChessUnitType.Queen)
            {
                throw new NotSupportedException($"QueenPathfinder doesn't support unit type {unit}");
            }
        ...
        private List<Vector2Int> GetNeighbors(Vector2Int position)
        {
            ...
            for (int x = -1; x <= 1; x++)
            {
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    neighbors.Add(new Vector2Int(position.x + x, position.y + y));
                }
            }
            return neighbors;
        }
